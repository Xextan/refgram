...
Like most logical languages, Xextan is almost entirely devoid of nouns as a distinct part of speech. Rather, most semantic concepts in Xextan are expressed in the form of a verb phrase or function, otherwise known as a [predicate](predicate). In order to be used as a noun, a predicate must first be converted into an [argument](argument).

This can be achieved in various ways, but the simplest is by placing a [case marker](case-marker) immediately before the predicate in question:
