...
Note that Xextan does not distinguish between definite and indefinite nouns. To specify that you are referring to a certain specific instance of something, possibly something recently mentioned, the demonstrative construction is used instead.
...
Adverbs and adverbial phrases formed with fi are typically written with a hyphen, as above; however, this is optional, and like all Xextan punctuation, its presence or absence does not directly affect the syntax of a sentence.
...
A determiner is a word which expresses the intended referent of a given word or phrase. In Xextanic terms, a determiner converts a predicate into a noun, or adds additional information to an existing noun. The most basic determiners in Xextan are numbers, or quantifiers. Although Xextan lacks a singular-plural distinction, you can convey this distinction by directly specifying a number. The numbers from zero to nine are nu (0), ne (1), duo (2), tie (3), kua (4), pei (5), lio (6), qai (7), bui (8), and giu (9).
...
Xextan also features a set of possessive determiners, which create a possessive noun phrase. These include nia (“my”/”our”), vua (“your”), lia (“his”/”her”/”their”), and gua (“its”).

pon (o) nia nandze (e) vua papkin (a) lia padqbu.
“My dad put your bag on his desk.”

The determiner po converts a predicate or noun into an abstraction, or a subordinate clause. As with all other determiners, the resulting phrase is treated as a noun. This is similar to how the infinitive and participle forms of English predicates are used to convert predicates into nouns, like how the predicate “work” (gun, in Xextan) becomes “to work” or “working” (po gun)
...
Perhaps the most useful and versatile of all is peu, the topic marker:
...
The preposition za is used to describe the location, context, or environment in which something takes place:
...
To state that an action was performed using a certain tool or implement or by a certain method or technique, the preposition *fo* is used.
...
pio tu buk, u xil vo su nit qa sin.
...
A few of the most common lambda predicates include ken (O is able to do Eu); tet (O tries to do Eu); vol (O wants to do Eu); txu (O needs to do Eu); and tse (O intends to do Eu). The “Eu” in the predicate definition conveys that the serialized predicate corresponds to a po-clause placed in the E case slot.
